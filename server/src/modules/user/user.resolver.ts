...
import { FollowUserInput, LoginUserInput, RegisterUserInput, User, UserFollowers } from "./user.dto";
import {
  createUser,
...
class UserResolver {
  ...
  @Authorized()
  @Mutation(() => User)
  async followUser(@Arg("input") input: FollowUserInput, @Ctx() context: Context) {
    try {
  ...
  @Authorized()
  @Mutation(() => User)
  async unfollowUser(@Arg("input") input: FollowUserInput, @Ctx() context: Context) {
